...
export default function SuperAdminDashboardPage() {
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6 md:p-8">
        ...
        >
          <div className="flex items-center gap-3">
            <Select value={selectedGarrisonId} onValueChange={setSelectedGarrisonId}>
                <SelectTrigger className="w-[280px] h-10 shadow-sm dark:shadow-none border-border">
                    <LayoutGrid className="size-4 mr-2 text-muted-foreground" />
                    <SelectValue placeholder="All Garrisons" />
                </SelectTrigger>
                <SelectContent className="rounded-xl border-border shadow-xl dark:shadow-none">
        ...
        {selectedGarrisonId === 'all' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {(filteredMetrics?.categoryRevenue || []).map((cat: any) => (
                    <Card key={cat.category} className="shadow-sm dark:shadow-none border-border">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold tracking-tight">{formatCurrency(cat.total)}</div>
                            <p className="text-xs text-muted-foreground mt-1">Verified {cat.category} collections</p>
                        </CardContent>
                    </Card>
                ))}
            </div>
        ...
        <Card className="shadow-sm dark:shadow-none border-border rounded-2xl overflow-hidden">
            <CardHeader className="border-b border-border bg-muted/20">
                <CardTitle className="text-base font-bold">Garrison Operational Breakdown</CardTitle>
                <CardDescription className="text-xs font-medium text-muted-foreground">Comparison of battalion strength and fee collections.</CardDescription>
            </CardHeader>
                    ...
                    <TableBody>
                        {(data?.garrisonsPerformance || []).map((g) => (
                            <TableRow key={g.garrison_id} className={selectedGarrisonId !== 'all' && String(selectedGarrisonId) !== String(g.garrison_id) ? 'opacity-30' : ''}>
                                <TableCell className="pl-6 font-bold">{g.garrison_name}</TableCell>
                                <TableCell className="text-center font-medium text-muted-foreground">{g.total_schools}</TableCell>
                                <TableCell className="text-center font-semibold text-foreground/80">{g.total_students}</TableCell>
